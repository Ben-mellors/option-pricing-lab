...
\Cref{fig:tree} shows the first few steps, and \cref{fig:conv-binomial} how the
resulting tree price behaves as the number of steps grows.
...
\begin{figure}[H]
  ...
  \end{tikzpicture}
  \caption{The first three steps of a recombining CRR tree. Each step moves up
    by a factor $u$ with risk-neutral probability $p$ or down by $d$ with
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.82\textwidth]{convergence_binomial.png}
  \caption{Binomial price against the number of steps $N$. The price converges
    to the Black--Scholes value with the characteristic odd--even oscillation,
    which decays as the step count grows.}
...
into two equal halves; the nearest price is more than £11 away.
...
difference between neighbouring grid values. The time derivative is
approximated by a forward difference,
\[
  \frac{\partial V}{\partial t}\bigg|_{(S_i,t_n)}
  \approx \frac{V_i^{\,n+1} - V_i^{\,n}}{\Delta t},
\]
and the two space derivatives by central differences at the same time level
$n+1$,
\[
  \frac{\partial V}{\partial S}\bigg|_{(S_i,t_{n+1})}
  \approx \frac{V_{i+1}^{\,n+1} - V_{i-1}^{\,n+1}}{2\,\Delta S}, \qquad
  \frac{\partial^2 V}{\partial S^2}\bigg|_{(S_i,t_{n+1})}
  \approx \frac{V_{i+1}^{\,n+1} - 2V_i^{\,n+1} + V_{i-1}^{\,n+1}}{(\Delta S)^2}.
\]
The choice to evaluate the space derivatives at $t_{n+1}$ rather than $t_n$ is
what makes the scheme \emph{explicit}: $V_i^{\,n}$, the one unknown, is being
solved for using only values already known at the later time $t_{n+1}$, with
no other unknowns appearing on the right. Substituting these three
approximations into \eqref{eq:bspde} and rearranging
...
\end{equation}
Here $i$ is the price-step index, so $S_i = i\,\Delta S$; the three
coefficients absorb both the $\Delta S$ from the space differences and the
$S_i = i\,\Delta S$ that multiplies the derivatives in \eqref{eq:bspde}
itself, which is why each depends on $i$ rather than being constant across
the grid. Each interior value at one time level is built from three values at
the next, as in the stencil of \cref{fig:stencil}.
...
below the threshold, the solution reaches a magnitude of around $10^{138}$
within a hundred time steps; with $N = 2000$, above it, the scheme is stable
and returns $10.4550$.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{pde_stability.png}
